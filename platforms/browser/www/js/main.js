...
var Propagandas;
var BancoDeDados = window.localStorage;
...
function consulta(){
    $.ajax({
        url: "https://feeds.feedburner.com/jornaloaperitivo/hkmN",
        type: "GET",
        data: "",
        dataType: "xml",
        success: function (Feed, textStatus, jqXHR) {
            ...
            for (var c = 0; c < entradas.length; c++) {
                ...
                Noticias.push({
                    ...
                    texto: aux,
                    link: entradas[c].getElementsByTagName("feedburner:origLink")[0].innerHTML,
...
function preencherTabela(min,max){
    ...
    for (var c = min; c < max; c++) {
      //corrigir LINK
      if (Noticias[c].link === undefined)noticias[c].link = 'https://jornal.jornaloaperitivo.com/';
      /// corrigir TITULO
...
$("#btnBusca").click(function () {/// mostra barra de busca
    if ($("#buscar").css('display') === "none") {
      ...
      $("#buscar input").val("");
      if (busca !== "")
          var ok = cordova.InAppBrowser.open("https://jornal.jornaloaperitivo.com/search?q=" + busca.replace(/ /g, "+"),"_system");
            console.log(ok);
...
$(document).keypress(function (e) {///FUNCAO PARA PESQUISAR COM O ENTER REPETIDA
    if (e.which === 13) {
        ...
        $("#buscar input").val("");
        if (busca !== "")
            cordova.InAppBrowser.open("https://jornal.jornaloaperitivo.com/search?q=" + busca.replace(/ /g, "+"),"_system");
